...
describe('Post e2e test', () => {
  ...
  let updatePage: PostUpdatePage;
  let detailsPage: PostDetailsPage;
  let listPage: PostComponentsPage;
  let deleteDialog: PostDeleteDialog;
  ...
  it('should load Posts', async () => {
    ...
    listPage = new PostComponentsPage();
  ...
  describe('Create flow', () => {
    it('should load create Post page', async () => {
      ...
      updatePage = new PostUpdatePage();

      await waitUntilAllDisplayed([updatePage.title, updatePage.footer, updatePage.saveButton]);

      expect(await updatePage.title.getAttribute('id')).to.match(/newsApp.post.home.createOrEditLabel/);
    });
    ...
    it('should create and save Posts', async () => {
      ...
      await updatePage.coverImageUrlInput.sendKeys('coverImageUrl');
      expect(await updatePage.coverImageUrlInput.getAttribute('value')).to.match(/coverImageUrl/);
    ...
    describe('Details, Update, Delete flow', () => {
      after(async () => {
        ...
        deleteDialog = new PostDeleteDialog();
        await waitUntilDisplayed(deleteDialog.dialog);

        expect(await deleteDialog.title.getAttribute('id')).to.match(/newsApp.post.delete.question/);
      ...
      it('should load details Post page and fetch data', async () => {
        const detailsButton = listPage.getDetailsButton(listPage.records.first());
        await click(detailsButton);

        detailsPage = new PostDetailsPage();
      ...
      it('should load edit Post page, fetch data and update', async () => {
        ...
        await updatePage.coverImageUrlInput.clear();
        await updatePage.coverImageUrlInput.sendKeys('modified');
        expect(await updatePage.coverImageUrlInput.getAttribute('value')).to.match(/modified/);
