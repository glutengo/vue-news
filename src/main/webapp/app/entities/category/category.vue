<template>
  <div>
    <h2 id="page-heading" data-cy="CategoryHeading">
      <span v-text="$t('newsApp.category.home.title')" id="category-heading">Categories</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('newsApp.category.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'CategoryCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('newsApp.category.home.createLabel')"> Create a new Category </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && categories && categories.length === 0">
      <span v-text="$t('newsApp.category.home.notFound')">No categories found</span>
    </div>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="newsApp.category.delete.question" data-cy="categoryDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-category-heading" v-text="$t('newsApp.category.delete.question', { id: removeId })">
